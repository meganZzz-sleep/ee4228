...
def get_embedding(face_img):
    """
    Given a face image (assumed to be preprocessed as needed),
    compute and return the embedding using FaceNet.
    """
    # Resize the face to the required size (if not already 160x160)
    face_img = cv2.resize(face_img, (160, 160))
    # Get the embedding; note that this might be heavy so consider caching or running in a thread.
            ...
            try:
                ...
                for face in faces:
                    if face['confidence'] > 0.8:
                        ...
                        face_img = align_face(face_img, keypoints)
